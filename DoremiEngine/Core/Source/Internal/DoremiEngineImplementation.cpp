...
namespace DoremiEngine
{
    namespace Core
    {
        ...
        void DoremiEngineImplementation::AssertThatSpecificLibraryExists(const std::string& p_libraryName)
        {
            ...
            m_logger->LogText(LogTag::ENGINE_CORE, LogLevel::INFO, "Attempt to detect %s", p_libraryName.c_str());

            if(handle != NULL)
            {
                m_logger->LogText(LogTag::ENGINE_CORE, LogLevel::INFO, "Attempt to detect %s - Successful", p_libraryName.c_str());
                FreeSharedLibrary(handle);
            }
            else
            {
                m_logger->LogText(LogTag::ENGINE_CORE, LogLevel::INFO, "Attempt to detect %s - Unsuccessful", p_libraryName.c_str());
            }
        }
